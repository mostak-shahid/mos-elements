<?php
function mos_elements_admin_enqueue_scripts(){
	$page = @$_GET['page'];
	...
	var_dump($page); //mos_elements_settings(If under settings)*/
	
	if ($pagenow == 'options-general.php' AND $page == 'mos_elements_settings') {
		wp_enqueue_style( 'mos-elements-admin', plugins_url( 'css/mos-elements-admin.css', __FILE__ ) );
...
function mos_elements_enqueue_scripts(){
	...
	wp_enqueue_style( 'mos-elements', plugins_url( 'css/mos-elements.css', __FILE__ ) );
	wp_enqueue_script( 'mos-elements-functions', plugins_url( 'js/mos-elements-functions.js', __FILE__ ), array('jquery') );
	wp_enqueue_script( 'mos-elements', plugins_url( 'js/mos-elements.js', __FILE__ ), array('jquery') );
}
add_action( 'wp_enqueue_scripts', 'mos_elements_enqueue_scripts' );
function mos_elements_ajax_scripts(){
	wp_enqueue_script( 'mos-elements-ajax', plugins_url( 'js/mos-elements-ajax.js', __FILE__ ), array('jquery') );
	$ajax_params = array(
		'ajax_url' => admin_url('admin-ajax.php'),
		'ajax_nonce' => wp_create_nonce('mos_elements_verify'),
	);
	wp_localize_script( 'mos-elements-ajax', 'ajax_obj', $ajax_params );
}
add_action( 'wp_enqueue_scripts', 'mos_elements_ajax_scripts' );
...
add_action( 'wp_footer', 'mos_elements_scripts', 100 );